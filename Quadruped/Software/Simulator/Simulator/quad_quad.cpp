...
#pragma region Leg
	...
	bool Leg::InitRF(ID3D11Device *device)
	{
		...
		mth::float3 rotOffset = { mth::pi * 0.0f, mth::pi * 0.0f, mth::pi * 0.0f };
		...
		rotOffset = { mth::pi * 0.0f, mth::pi * 0.0f, mth::pi * 0.0f };

		if (!m_thigh.Init(device, L"resources/Robot/LegMiddle.mdl", mth::float4(1.0f)/*, rotOffset, posOffset*/))
			return false;
		...
		if (!m_toe.Init(device, L"resources/Robot/LegEnd.mdl", mth::float4(1.0f)/*, rotOffset, posOffset*/))
			return false;
	...
	bool Leg::InitLF(ID3D11Device *device)
	{
		...
		if (!m_shouder.Init(device, L"resources/Robot/LegStart_MIR.mdl", mth::float4(1.0f)/*, rotOffset, posOffset*/))
			return false;
		m_shouder.position = { m_ox, m_oy, m_oz };
		m_shouder.setColor(1.0f);
		m_shouder.rotation.y = m_a1;
		...
		if (!m_thigh.Init(device, L"resources/Robot/LegMiddle_MIR.mdl", mth::float4(1.0f)/*, rotOffset, posOffset*/))
			return false;
		m_thigh.position = { 0.0f, 0.0f, m_o1 };
		m_thigh.setColor(1.0f);
		m_thigh.rotation.x = m_a2;
		...
		if (!m_toe.Init(device, L"resources/Robot/LegEnd_MIR.mdl", mth::float4(1.0f)/*, rotOffset, posOffset*/))
			return false;
		m_toe.position = { m_o3x, 0.0f, m_o2 };
		m_toe.setColor(1.0f);
		m_toe.rotation.x = m_a3;
