...
#pragma region WalkScript
	...
	*/
	void WalkScript::AddPathElementMove(mth::float2 relativePos, float relativeHeadding)
	{
		...
		calculateOptimalsteps(&legStretchHalf, &turnAtOnce, distance, 0);
...
#pragma region WalkManager
	...
	void WalkManager::Update(float deltaTime)
	{
		if (!m_running)
		{
			...
			m_script.AddPathElementMove(mth::float2(2.0f, 0.0f), mth::pi * 0.0f);
			m_script.AddPathElementMove(mth::float2(-1.0f, -2.0f), mth::pi * 0.5f);
